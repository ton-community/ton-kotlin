...
import org.ton.block.account.AccountStatus
...
private object TransactionTlConstructor : TlbConstructor<Transaction>(
    ...
    override fun loadTlb(cellSlice: CellSlice): Transaction = cellSlice {
        ...
        val origStatus = loadTlb(AccountStatus.Tlb)
        val endStatus = loadTlb(AccountStatus.Tlb)
        val r1 = loadRef(TransactionAux)
        val totalFees = loadTlb(CurrencyCollection.Tlb)
        val stateUpdate = loadRef(HashUpdate.Companion)
        val description = loadRef(TransactionDescr.Companion)
        Transaction(
            accountAddr,
            lt,
            prevTransHash,
    ...
    override fun storeTlb(cellBuilder: CellBuilder, value: Transaction) = cellBuilder {
        ...
        storeTlb(AccountStatus.Tlb, value.origStatus)
        storeTlb(AccountStatus.Tlb, value.endStatus)
        storeRef(TransactionAux, value.r1)
        storeTlb(CurrencyCollection.Tlb, value.totalFees)
        storeRef(HashUpdate.Companion, value.stateUpdate)
        storeRef(TransactionDescr.Companion, value.description)
    }
}
...
private object TransactionAuxTlbConstructor : TlbConstructor<TransactionAux>(
    ...
    val maybeMessage = Maybe.Companion.tlbCodec(CellRef.tlbCodec(Message.Companion.Any))
    val outMsgs = HashMapE.tlbCodec(15, CellRef.tlbCodec(Message.Companion.Any))
