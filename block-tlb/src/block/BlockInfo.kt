...
import org.ton.block.BlkPrevInfo
import org.ton.block.GlobalVersion
...
public data class BlockInfo(
    ...
    @SerialName("master_ref") val masterRef: CellRef<BlockRef>?, // master_ref : not_master?^BlkMasterInfo
    @SerialName("prev_ref") val prevRef: CellRef<BlkPrevInfo>, // prev_ref : ^(BlkPrevInfo after_merge)
    @SerialName("prev_vert_ref") val prevVertRef: CellRef<BlockRef>? // prev_vert_ref : after_merge?^(BlkPrevInfo 0)
) : TlbObject {
...
private object BlockInfoTlbConstructor : TlbConstructor<BlockInfo>(
    ...
    private val blkMasterInfo = CellRef.tlbCodec(BlockRef.Tlb)
    ...
    override fun storeTlb(
        ...
        storeTlb(ShardIdent.Companion, value.shard)
        ...
        if (value.flags and 1 != 0 && value.genSoftware != null) {
            storeTlb(GlobalVersion.Tlb, value.genSoftware)
        }
        if (value.notMaster && value.masterRef != null) {
            storeTlb(blkMasterInfo, value.masterRef)
        }
        storeTlb(CellRef.tlbCodec(BlkPrevInfo.Companion.tlbCodec(value.afterMerge)), value.prevRef)
        if (value.vertSeqnoIncr && value.prevVertRef != null) {
            storeTlb(blkMasterInfo, value.prevVertRef)
        }
    }
    ...
    override fun loadTlb(
        ...
        val shard = loadTlb(ShardIdent.Companion)
        ...
        val genSoftware = if (flags and 1 != 0) loadTlb(GlobalVersion.Tlb) else null
        val masterRef = if (notMaster) loadTlb(blkMasterInfo) else null
        val prevRef = loadTlb(CellRef.tlbCodec(BlkPrevInfo.Companion.tlbCodec(afterMerge)))
        val prevVertRef = if (verSeqnoIncr) loadTlb(blkMasterInfo) else null
        BlockInfo(
            version,
            notMaster,
            afterMerge,
