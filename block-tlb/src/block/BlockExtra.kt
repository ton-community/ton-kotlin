...
import org.ton.block.McBlockExtra
import org.ton.block.currency.CurrencyCollection
import org.ton.block.message.ImportFees
import org.ton.block.message.export.OutMsg
...
private object BlockExtraTlbConstructor : TlbConstructor<BlockExtra>(
    ...
    val outMsgDescr = CellRef.tlbCodec(HashmapAugE.tlbCodec(256, OutMsg, CurrencyCollection.Tlb))
    val shardAccountBlock = CellRef.tlbCodec(
        HashmapAugE.tlbCodec(
            256,
            AccountBlock.Companion,
            CurrencyCollection.Tlb
        )
    )
    val maybeMcBlockExtra = Maybe.Companion.tlbCodec(CellRef.tlbCodec(McBlockExtra.Companion))
