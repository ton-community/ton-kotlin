    ...
    override fun encode(
        ...
        when (value) {
            is HashMapLabelSame -> {
                ...
                HashMapLabelSame.tlbCodec.encode(cellWriter, value, typeParam, param, negativeParam)
            }
            is HashMapLabelLong -> {
                cellWriter.writeBits(true, false)
                HashMapLabelLong.tlbCodec.encode(cellWriter, value, typeParam, param, negativeParam)
            }
            is HashMapLabelShort -> {
                cellWriter.writeBits(false)
                HashMapLabelShort.tlbCodec.encode(cellWriter, value, typeParam, param, negativeParam)
            }
        }
    }
    ...
    override fun decode(
        ...
        return if (cellReader.readBit()) {
            if (cellReader.readBit()) {
                HashMapLabelSame.tlbCodec.decode(cellReader, typeParam, param, negativeParam)
            } else {
                HashMapLabelLong.tlbCodec.decode(cellReader, typeParam, param, negativeParam)
            }
        } else {
            HashMapLabelShort.tlbCodec.decode(cellReader, typeParam, param, negativeParam)
        }
    }
}
...
object HashMapLabelShortTlbConstructor : TlbConstructor<HashMapLabelShort>(
    ...
    override fun encode(
        ...
        var n = 0
        Unary.tlbCodec.encode(cellWriter, value.len, typeParam, param) { n = it }
    ...
    override fun decode(
        ...
        var n = 0
        val len = Unary.tlbCodec.decode(cellReader) { n = it }
