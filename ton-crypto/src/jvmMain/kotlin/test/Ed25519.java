...
public abstract class Ed25519 {
    ...
    // "SigEd25519 no test.Ed25519 collisions"
...
//        assert 0 <= index && index < PRECOMP_POINTS;
        ...
        for (int i = 0; i < PRECOMP_POINTS; ++i) {
            ...
            off += F.SIZE;
            F.cmov(cond, precompBase, off, p.ymx_h, 0);
            off += F.SIZE;
            F.cmov(cond, precompBase, off, p.xyd, 0);
            off += F.SIZE;
        }
    }
...
//                    F.normalize(r.xyd);
                    ...
                    off += F.SIZE;
                    F.copy(r.ymx_h, 0, precompBase, off);
                    off += F.SIZE;
                    F.copy(r.xyd, 0, precompBase, off);
                    off += F.SIZE;
                }
            }
