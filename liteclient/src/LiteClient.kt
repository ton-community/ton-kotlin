...
import org.ton.kotlin.message.address.VarAddr
...
public class LiteClient(
...
) : Closeable, CoroutineScope, Provider {
    init {
    ...
    private var lastMasterchainBlockId: TonNodeBlockIdExt? by atomic(null)
    ...
    override suspend fun getLastBlock(): BlockId {
        val masterchainInfo = liteApi.sendQuery(
            LiteServerGetMasterchainInfo,
            LiteServerMasterchainInfo,
            LiteServerGetMasterchainInfo
        )
        var current = lastMasterchainBlockId
        if (current == null) {
            lastMasterchainBlockId = masterchainInfo.last
            current = masterchainInfo.last
        } else if (current.seqno < masterchainInfo.last.seqno) {
            lastMasterchainBlockId = masterchainInfo.last
            current = masterchainInfo.last
        }

        return current.toBlockId()
    }
    ...
    override suspend fun getAccountState(
        ...
        val currentBlockId = blockId?.toLiteApi() ?: lastMasterchainBlockId ?: getLastBlock().toLiteApi()
        val rawAccountState = liteApi(
            LiteServerGetAccountState(currentBlockId, address.toLiteApi()),
        )
    ...
    override suspend fun sendMessage(
        message: Message<ExtInMsgInfo, CellSlice>,
        context: CellContext
    ) {
        TODO("Not yet implemented")
    }
...
//    }
//
//    override suspend fun getAccountState(accountAddress: IntAddr): FullAccountState =
//        getAccountState(accountAddress, getLastBlockId())
//
//    public override suspend fun getAccountState(
//        accountAddress: IntAddr, blockId: TonNodeBlockIdExt
//    ): FullAccountState {
...
//    }
//
//    private fun IntAddr.toLiteServer() = LiteServerAccountId(workchain, ByteString(*address.toByteArray()))
//
//    override suspend fun getLastBlock(): BlockId = getLastBlockId().toBlockId()

}

internal fun IntAddr.toLiteApi() = LiteServerAccountId(workchain, ByteString(*address.toByteArray()))

internal fun LiteServerAccountId.toIntAddr(): IntAddr =
    if (id.size == 32) StdAddr(workchain, id) else VarAddr(workchain, id)

internal fun TonNodeBlockIdExt.toBlockId(): BlockId {
    val shardIdent = ShardIdent(
        workchain,
        shard.toULong()
    )
    return BlockId(
        shardIdent, seqno, rootHash, fileHash
    )
}

internal fun BlockId.toLiteApi(): TonNodeBlockIdExt =
    TonNodeBlockIdExt(shard.workchain, shard.prefix.toLong(), seqno, rootHash, fileHash)